...
- **Invite links are the credential.** Accepting an invite no longer requires the recipient to sign in with the exact invited email — whoever opens the (secret, single-use, expiring) link and signs in joins the camp. This is deliberate: it removes the "sent to a different email address" lockouts. Founders can't consume invites (they're told to open the camp from the console instead).
