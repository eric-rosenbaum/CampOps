            ...
            VStack(alignment: .leading, spacing: Spacing.xl) {
                ...
                if let url = vm.issue.photoUrl, let imageUrl = URL(string: url) {
                    AsyncImage(url: imageUrl) { phase in
                        ...
                        default: Color(.systemGray5).frame(height: 200).cornerRadius(Radius.md)
                        }
                    }
                }
                // Description
                ...
                if vm.issue.status != .resolved {
                    VStack(spacing: Spacing.sm) {
                        ...
                        Button { showingResolveSheet = true } label: {
                            Label("Mark Resolved", systemImage: "checkmark.circle").frame(maxWidth: .infinity)
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section {
                    Button("Mark as Resolved") { onResolve(Double(costInput)); dismiss() }
                        .foregroundColor(.sage)
                }
            }
            .navigationTitle("Resolve Issue").navigationBarTitleDisplayMode(.inline)
