            ...
            List {
                Section {
                    HStack(spacing: Spacing.md) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text(userManager.currentUser.name).font(.headline)
                ...
                Section {
                    ForEach(CampUser.seedUsers) { user in
                        Button {
                            userManager.switchUser(to: user); dismiss()
                        } label: {
                            HStack(spacing: Spacing.md) {
                    ...
                    Text("Switching users demonstrates role-based permissions. Replace switchUser() with Supabase auth when ready.")
                        .font(.caption)
                }
            }
