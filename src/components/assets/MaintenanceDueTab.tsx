  ...
  if (overdue.length === 0 && soon.length === 0 && scheduled.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-body text-ink-faint">No service logged with a next service date.</p>
      </div>
    );
  }
...
function Section({ title, count, urgent = false, upcoming = false, children }: { title: string; count: number; urgent?: boolean; upcoming?: boolean; children: React.ReactNode }) {
  const badgeClass = urgent ? 'bg-red/10 text-red' : upcoming ? 'bg-forest/8 text-ink-soft' : 'bg-amber-bg text-amber-text';
  return (
    <div>
...
function ServiceRow({
  ...
  const dateClass = isOverdue ? 'text-red font-medium' : upcoming ? 'text-ink-soft' : 'text-amber-text';
  ...
  return (
    <div className={`bg-white border rounded-card px-4 py-3 border-l-[3px] flex items-center justify-between gap-3 ${borderClass}`}>
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 text-label font-medium rounded-btn border border-border hover:border-sage hover:bg-sage/5 text-ink-soft hover:text-forest transition-colors flex-shrink-0"
      >
