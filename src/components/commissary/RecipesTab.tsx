...
function RecipeCard({ recipeId }: { recipeId: string }) {
  ...
  return (
    <div className="bg-white rounded-card border border-border overflow-hidden">
      ...
      >
        {expanded ? <ChevronDown className="w-4 h-4 text-ink-faint" /> : <ChevronRight className="w-4 h-4 text-ink-faint" />}
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-border pt-4">
          <div className="flex items-center gap-2 mb-4">
            <label className="text-[12px] text-ink-soft">Scale to</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-ink-faint mb-2">Ingredients</p>
                ...
                {ings.map((ing) => {
                  ...
                  return (
                    <div key={ing.id} className="flex items-center justify-between gap-3 text-[12px]">
                      ...
                      <span className={`font-mono flex-shrink-0 ${unlinked ? 'text-ink-faint' : 'text-forest'}`}>
                        {scaledIngredientLabel(ing, recipe, effective, item)}
                      </span>
                    </div>
                  );
                })}
                {ings.length === 0 && <p className="text-[12px] text-ink-faint">No ingredients yet.</p>}
              </div>
            ...
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-ink-faint mb-2">Method</p>
              {steps.length > 0 ? (
                <ol className="space-y-2">
                  {steps.map((s) => {
                    const timing = stepTimingLabel(s);
                    return (
                      <li key={s.id} className="flex gap-2.5 text-[12px] text-ink">
                        <span className="font-mono text-ink-faint flex-shrink-0">{s.stepNumber}.</span>
                        <span className="leading-relaxed">
                          {timing && (
                            <span className="inline-block mr-1.5 px-1.5 py-0.5 rounded-tag text-[10px] font-medium bg-forest/8 text-ink align-middle">
                              {timing}
                            </span>
                          )}
                          {s.instruction}
                ...
                <p className="text-[12px] text-ink leading-relaxed whitespace-pre-wrap">{recipe.method}</p>
              ) : (
                <p className="text-[12px] text-ink-faint">No method recorded.</p>
              )}
            </div>
          </div>
        </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 sm:px-7 py-4 sm:py-6">
      ...
      <div className="space-y-2.5">
        ...
        {rows.length === 0 && (
          <p className="px-4 py-8 text-center text-[13px] text-ink-faint">No recipes match this filter.</p>
        )}
      </div>
    </div>
  );
