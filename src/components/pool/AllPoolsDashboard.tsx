...
function PoolStatusCard({ pool }: { pool: CampPool }) {
  ...
  const statusBadge = () => {
    if (!isWaterfront) {
      if (chemStatus === 'none') return { label: 'No readings', cls: 'bg-cream-dark text-ink-faint' };
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-meta text-ink-faint mt-0.5">{POOL_TYPE_LABELS[pool.type]}</p>
        </div>
      ...
      {!isWaterfront && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
          {latest ? (
            <>
              <div>
                <p className="text-meta text-ink-faint mb-0.5">Free Cl</p>
              ...
              <div>
                <p className="text-meta text-ink-faint mb-0.5">pH</p>
              ...
              <div>
                <p className="text-meta text-ink-faint mb-0.5">Last reading</p>
            ...
            <div className="col-span-3">
              <p className="text-meta text-ink-faint">No chemical readings yet</p>
            </div>
          )}
        </div>
      )}
      ...
      {isWaterfront && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
          <div>
            <p className="text-meta text-ink-faint mb-0.5">Equipment</p>
            <p className="text-body font-mono font-semibold text-forest">
          ...
          <div>
            <p className="text-meta text-ink-faint mb-0.5">Type</p>
      ...
      {pool.notes && (
        <p className="text-meta text-ink-faint border-t border-cream-dark pt-2 mt-1">{pool.notes}</p>
      )}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 mb-6">
        <div className="bg-white border border-border rounded-card px-4 py-4">
          <p className="text-meta font-semibold uppercase tracking-wide text-ink-faint">Total locations</p>
          <p className="font-mono text-stat font-semibold text-forest mt-1">{activePools.length}</p>
          <p className="text-meta text-ink-faint mt-0.5">Active this season</p>
        </div>
        <div className="bg-white border border-border rounded-card px-4 py-4">
          <p className="text-meta font-semibold uppercase tracking-wide text-ink-faint">Chemical pools</p>
          <p className="font-mono text-stat font-semibold text-forest mt-1">{chemPools.length}</p>
          <p className="text-meta text-ink-faint mt-0.5">Require daily readings</p>
        </div>
        <div className="bg-white border border-border rounded-card px-4 py-4">
          <p className="text-meta font-semibold uppercase tracking-wide text-ink-faint">Waterfront</p>
          <p className="font-mono text-stat font-semibold text-forest mt-1">{waterfrontPools.length}</p>
          <p className="text-meta text-ink-faint mt-0.5">Waterfront locations</p>
        </div>
      </div>
