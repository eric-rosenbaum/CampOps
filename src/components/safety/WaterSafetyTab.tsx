    ...
    <div className={`bg-white border border-border border-l-[3px] ${borderCls} rounded-card px-5 py-4`}>
        ...
        <div className="text-right flex-shrink-0 flex items-start gap-2">
          <div>
            {hasExpired ? (
              ...
              <span className="text-label font-semibold px-2.5 py-1 rounded-tag uppercase tracking-wide bg-cream-dark text-ink-faint">No certs</span>
            )}
            {lgCert?.expiryDate && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3 pt-3 border-t border-cream-dark">
        {LIFEGUARD_CERT_TYPES.map((type) => {
          ...
          return (
            <div key={type}>
              <p className="text-[10px] text-ink-faint font-medium mb-1">{CERT_TYPE_LABELS[type]}</p>
              <CertPill
                cert={cert}
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 mb-6">
        <div className="bg-white border border-border rounded-card px-4 py-4">
          <p className="text-meta font-semibold uppercase tracking-wide text-ink-faint">Lifeguards</p>
        ...
        <div className="bg-white border border-border rounded-card px-4 py-4">
          <p className="text-meta font-semibold uppercase tracking-wide text-ink-faint">Expiring certs</p>
        ...
        <div className="bg-white border border-border rounded-card px-4 py-4">
          <p className="text-meta font-semibold uppercase tracking-wide text-ink-faint">Expired certs</p>
          <p className={`font-mono font-semibold text-stat mt-1 ${lgSummary.expired > 0 ? 'text-red' : 'text-forest/30'}`}>{lgSummary.expired}</p>
          <p className={`text-meta mt-0.5 ${lgSummary.expired > 0 ? 'text-red' : 'text-ink-faint'}`}>
      ...
      {lifeguards.length === 0 ? (
        <div className="bg-white border border-border rounded-card px-5 py-8 text-center mb-6">
          <p className="text-[13px] text-ink-faint">No lifeguard certifications on file.</p>
          {can('manageSafetyStaff') && (
