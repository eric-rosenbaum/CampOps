// Sends the one-time code that unlocks the private half of a guest portal.
//
// The link stays password-free, because that is why coordinators use the portal at all. What
// it no longer opens on its own is the material that would matter if the link were forwarded:
// the named roster, who is sleeping where, invoices, and the agreement. Dates, the menu, the
// checklist and confirming a headcount stay open, since friction there costs completion and
...
// the visitor types, which would verify nothing. The response returns only a masked hint, so
// the portal can say "we sent a code to j•••@church.org" without disclosing the address to
...
/** Matches portal_link_expired() in the database. */
...
Deno.serve(async (req) => {
  ...
  // Checked here too, so an expired link cannot even trigger an email.
  const expiredAfter = new Date(retreat.departure_date);
  expiredAfter.setDate(expiredAfter.getDate() + LINK_TTL_DAYS);
  ...
  await admin.from("retreat_access_codes")
    .update({ consumed_at: new Date().toISOString() })
    .eq("retreat_id", retreat.id).is("consumed_at", null);

  const { error: insErr } = await admin.from("retreat_access_codes").insert({
  ...
  const html = `
    <p>Hello,</p>
    <p>Your access code for the <strong>${retreat.group_name}</strong> retreat portal is:</p>
    <p style="font-size:26px;font-weight:700;letter-spacing:5px;margin:20px 0">${code}</p>
    <p>It unlocks your guest list, room assignments, invoices and agreement for 12 hours on this
       device. It expires in ${CODE_TTL_MINUTES} minutes.</p>
    <p>If you did not request this, you can ignore this email. Nothing has been shared.</p>`;
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: email,
      subject: `Your portal access code: ${code}`,
      html,
    }),
  });
