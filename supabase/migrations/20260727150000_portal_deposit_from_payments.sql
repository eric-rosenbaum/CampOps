-- deposit_received on retreats is unused; the camp records deposits as retreat_payments
-- (kind='deposit') via the Payments modal. Derive deposit-paid from those so the portal
-- deposit card reflects reality ("dates held" once the deposit payment is recorded).
CREATE OR REPLACE FUNCTION get_portal_data(p_token text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE r retreats; v_charges numeric; v_paid numeric; v_deposit numeric;
BEGIN
