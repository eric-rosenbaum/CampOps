-- Portal account balance: when no manual charges exist, bill = rate × people × nights
-- (per pricing model). Also expose the rate + nights so the portal can show the breakdown.
...
DECLARE r retreats; v_charges numeric; v_paid numeric; v_deposit numeric; v_camp text;
        v_nights int; v_expected numeric;
BEGIN
