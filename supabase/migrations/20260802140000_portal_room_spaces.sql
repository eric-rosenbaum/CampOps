-- Guest portal: offer rooms as well as whole buildings. `spaces` now includes retreat-available
-- dorm buildings AND the active rooms (child locations) under them, labeled "Building · Room".
-- Blocked rooms are made inactive in Camp Info, so is_active filtering excludes them.
  ...
  RETURN jsonb_build_object(
    ...
    'spaces', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', l.id,
        'name', CASE WHEN l.parent_id IS NOT NULL
                     THEN COALESCE((SELECT p.name FROM locations p WHERE p.id = l.parent_id) || ' · ', '') || l.name
                     ELSE l.name END,
        'bed_capacity', l.bed_capacity, 'accessible', l.accessible) ORDER BY l.sort_order, l.name)
      FROM locations l
      WHERE l.camp_id = r.camp_id AND l.is_active AND (
        (l.is_dorm AND l.retreat_available)
        OR l.parent_id IN (SELECT d.id FROM locations d WHERE d.camp_id = r.camp_id AND d.is_dorm AND d.retreat_available AND d.is_active)
      )), '[]'::jsonb),
