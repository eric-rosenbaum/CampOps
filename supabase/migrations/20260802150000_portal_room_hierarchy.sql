-- Portal spaces as a building→room hierarchy: return ROOMS (child locations of retreat-available
-- dorms) each carrying its building id + name + bed count. Dorms with no rooms fall back to a
-- single space = the building itself. Beds live on rooms.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'spaces', COALESCE((
      SELECT jsonb_agg(sp ORDER BY sp->>'building', sp->>'name') FROM (
        SELECT jsonb_build_object('id', rm.id, 'name', rm.name,
          'building_id', d.id, 'building', d.name,
          'bed_capacity', rm.bed_capacity, 'accessible', rm.accessible) AS sp
        FROM locations d
        JOIN locations rm ON rm.parent_id = d.id AND rm.is_active
        WHERE d.camp_id = r.camp_id AND d.is_dorm AND d.retreat_available AND d.is_active
        UNION ALL
        SELECT jsonb_build_object('id', d.id, 'name', d.name,
          'building_id', d.id, 'building', d.name,
          'bed_capacity', d.bed_capacity, 'accessible', d.accessible) AS sp
        FROM locations d
        WHERE d.camp_id = r.camp_id AND d.is_dorm AND d.retreat_available AND d.is_active
          AND NOT EXISTS (SELECT 1 FROM locations rm WHERE rm.parent_id = d.id AND rm.is_active)
      ) t), '[]'::jsonb),
