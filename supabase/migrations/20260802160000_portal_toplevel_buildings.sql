-- A building is a TOP-LEVEL dorm (parent_id IS NULL); a room is its direct child. Portal spaces:
-- rooms of retreat-available top-level dorms where the ROOM itself is retreat_available + active
-- (so blocked rooms drop out); top-level dorms with no child rooms fall back to the building itself.
-- This stops a nested "Room 1" (a child dorm) from appearing as a building.
