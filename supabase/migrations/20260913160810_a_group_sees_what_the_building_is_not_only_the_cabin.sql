-- The portal returned rooms carrying their building's NAME and nothing else about it.
--
-- A camp can describe Boys Village -- "eight cabins around a central green, bathhouse a short
-- walk, no heating" -- and a group choosing cabins inside it never saw a word of that, because the
-- payload only carried descriptions for the leaves. The thing that decides whether a group wants
-- the village at all was the thing we dropped.
