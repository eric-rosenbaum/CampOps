-- `space_replies` existed because an approval had nowhere to live except the request row: the
-- portal scraped answered requests that happened to carry a sentence and drew a banner from
-- them. Now those answers are messages in a thread, and the only thing the checklist needs to
-- know is how many the group has not read.
