-- "Approved Craft Shop" posted a third time tells the group nothing, and reads as a bug. What
-- they need to know after editing a note is that the camp saw it.
--
-- Folded into the same function as the duplicate-job fix, because both come from the same
-- change: re-approving went from rare to routine.
