-- Two fixes to what approving a meeting space produces.
--
-- The checklist: it applied "Program space reset" flat, whatever the space was, so a job for the
-- Barn listed six steps and never mentioned that the Barn is a main room AND a bathroom. It now
-- builds a section per room for a space that has rooms inside it, taking each room's own
-- checklist where the camp set one.
--
-- And the body claimed "They have the room for the whole stay; set it up once." on every
-- multi-day booking. Since every booking is now the whole stay, it said it every time, which is
-- noise on a work order rather than information.
...
begin
  ...
  if v_setup is not null then
    ...
    -- A space with rooms inside it gets a section per room. The Barn is a main room and a
    -- bathroom, and a crew that ticks six steps has done half of it.
    perform public.append_space_steps(v_setup, r.location_id, v_tmpl);
  end if;
  ...
  if v_strike is not null then
    ...
    perform public.append_space_steps(v_strike, r.location_id, v_tmpl);
  end if;
